...
DLLEXPORT
int jl_save_incremental(const char *fname, jl_array_t *worklist)
{
    ios_t f;
    if (ios_file(&f, fname, 1, 1, 1, 1) == NULL) {
...
DLLEXPORT
jl_array_t *jl_restore_incremental_from_buf(const char *buf, size_t sz)
{
    ios_t f;
...
DLLEXPORT
jl_array_t *jl_restore_incremental(const char *fname)
{
    ios_t f;
    if (ios_file(&f, fname, 1, 0, 0, 0) == NULL) {
